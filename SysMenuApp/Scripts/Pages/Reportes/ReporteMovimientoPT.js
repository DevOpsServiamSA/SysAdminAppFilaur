var app_ReporteMovimientosPT = (function (win, doc) {

    const data = {
        urlGenerarReporte: '/SysMenuApp/Reporte/GenerarReportePT'
        //urlGenerarReporte: '/Reporte/GenerarReportePT'
    }

    function init() {
        document.getElementById("btnExportarMovimientoPT").style.display = "none";
        formatoRangoFecha();
        document.getElementById("btnBuscarMovimientoPT").addEventListener("click", listarMovimientoPT);
        document.getElementById("btnExportarMovimientoPT").addEventListener("click", ExportarExcel);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.lstReporte.length > 0) {
                    ...
                    htmls += "</table>";
                    var ctx = {
                        worksheet: 'MovimientoPT',
                        table: htmls
                    }
                    var link = document.createElement("a");
                    link.download = "ReporteMovimientoPT.xls";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (rpta.length > 0) {
                    ...
                    document.getElementById("btnExportarMovimientoPT").style.display = "block";
                    toastr.success('Registros obtenidos correctamente');
                } else {
                    $('#tbListaMovimientoPT').DataTable().clear().destroy();
    ...
    function crearTabla(data) {
        ...
        const table = `<table id="tbListaMovimientoPT" class ="table table-bordered table-hover" style="width:1350px">${thead}<tbody id="bodyTbListaMovimientoPT">${tbody}</tbody></table>`;
        document.getElementById("div_tbListaMovimientoPT").innerHTML = table;
        fotmatTable();
    }

    function fotmatTable() {
        $('#tbListaMovimientoPT').DataTable({
            "order": [[3, "asc"]],
            "language": {
